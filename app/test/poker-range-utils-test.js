...
describe('poker-range-utils', function() {
    ...
    describe('find_all_offsuit', function() {
        it ('should find 12 pairs and each card in the pair must have a different suit', function() {
            ...
            pairs[0].name.should.eql('AhKc');
            pairs[1].name.should.eql('AhKd');
            pairs[2].name.should.eql('AhKs');
            pairs[3].name.should.eql('AcKh');
            pairs[4].name.should.eql('AcKd');
            pairs[5].name.should.eql('AcKs');
            pairs[6].name.should.eql('AdKh');
            pairs[7].name.should.eql('AdKc');
            pairs[8].name.should.eql('AdKs');
            pairs[9].name.should.eql('AsKh');
            pairs[10].name.should.eql('AsKc');
            pairs[11].name.should.eql('AsKd');
        });
    });
});
